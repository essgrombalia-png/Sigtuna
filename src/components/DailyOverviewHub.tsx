...
import { Quote, Shuffle, Gauge, Zap, Calendar, ChevronRight, Sparkles, Copy, Check, RefreshCw } from 'lucide-react';
...
export const DailyOverviewHub: React.FC<DailyOverviewHubProps> = ({
  ...
  // Morning Quote state
  const [currentQuote, setCurrentQuote] = useState<MorningQuote>(() => getDailyQuote());
  ...
  const handleCopyQuote = async (e: React.MouseEvent) => {
    e.stopPropagation();
    const textToCopy = `"${currentQuote.quote}" — ${currentQuote.author || 'Sigtuna kommun'}`;
    try {
  ...
  return (
    <div className="w-full max-w-xl mx-auto">
      <div className="ios-glass-card rounded-2xl p-3 sm:p-3.5 shadow-sm border border-blue-200/60 dark:border-blue-900/50 space-y-2 transition-all">
        
        {/* Top: Dagens Inspirerande Citat med kategori, kopiera & hämta nytt */}
        <div className="space-y-1.5">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wider bg-blue-100/90 dark:bg-blue-950/90 text-blue-900 dark:text-blue-200 border border-blue-200/60 dark:border-blue-800/60 shadow-2xs">
                <Sparkles className="w-3 h-3 text-amber-500 icon-realistic" />
                <span>Dagens inspirerande citat</span>
              </span>

              {currentQuote.category && (
                <span className="hidden xs:inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-50/90 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-200/60 dark:border-amber-800/50 shadow-2xs">
                  {currentQuote.category}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                disabled={isChanging}
                title="Hämta ett nytt inspirerande citat"
                aria-label="Hämta ett nytt inspirerande citat"
              ...
              >
                <RefreshCw className={`w-3 h-3 icon-realistic transition-transform duration-300 ${isChanging ? 'animate-spin' : 'group-hover:rotate-180'}`} />
          ...
          <div className="flex items-start gap-2.5 pt-0.5">
            ...
            <div className="flex-1 min-w-0 text-left">
              <p
                className={`text-xs sm:text-sm text-slate-700 dark:text-slate-200 italic font-medium leading-relaxed transition-opacity duration-150 ${
              ...
              {currentQuote.author && (
                <p className="text-[10px] sm:text-[11px] font-semibold text-slate-500 dark:text-slate-400 mt-0.5 text-right">
                  — {currentQuote.author}
                </p>
              )}
            </div>
